...
\label{cap:atividades}
  ...
  O \SMgr{} é a segunda principal classe da interface comum entre linguagens, e praticamente a
  única outra classe que o usuário precisa usar além dos \VObj{s}. Ela segue o padrão de projeto
  \textbf{\textit{singleton}}\footnotemark{} e como explicado na seção \ref{cap:estrutura:opa}
  gerencia as máquinas virtuais que estão disponíveis no sistema, permitindo ao usuário 
  ...
  cada linguagem que se deseja ser compatível com a OPA, como falado na seção \ref{cap:estrutura:opa}.
  A \VMac{} representa a máquina virtual da linguagem em si, sendo usada internamente pelo 
  \SMgr{}. Raramente ela será usada diretamente pelo usuário. Alguns métodos importantes de 
  sua interface são:
