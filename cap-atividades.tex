...
\label{cap:atividades}
  ...
  e \textit{open-source}. Disponível em: \url{http://www.cmake.org} (último acesso: 22/11/2013)}
  ...
  \url{http://www.debian.org} (último acesso: 22/11/2013)} ou Ubuntu\footnote{Outra distribuição de Linux. 
  Disponível em: \url{http://www.ubuntu.com} (último acesso: 22/11/2013)}) usando \lang{g++}\footnote{
  Compilador para C++ do projeto GNU. Disponível em: \url{http://gnu.gcc.org} (último acesso: 22/11/2013)}
  ...
  compilador e outras ferramentas. Mais informações: \url{http://www.visualstudio.com/}. 
  (último acesso: 22/11/2013)} 2010. Para controle de versão do código fonte compartilhávamos o repositório 
  da UGDK, que usava Git\footnote{Ferramenta de controle de versão livre e \textit{open-source}.
  Disponível em: \url{http://git-scm.com/} (último acesso em: 22/11/2013)}, hospedado no GitHub\footnote{
  Serviço de hospedagem de repositórios Git. Acessível em: \url{https://github.com/} 
  (último acesso: 22/11/2013)}.
  ...
  operações do \VObj{} são repassadas para a instância \VData{} usando um padrão de projeto chamado
  ...
  \begin{enumerate}
    %TODO achar um outro nome para Cabeçalho... acaba ficando confuso com headers...
    \item \textbf{Cabeçalho}: fornecido pela especificação usada, esse bloco deve conter
    ...
    \item \textbf{Central}: fornecido pela especificação usada, esse bloco é similar ao 
      cabeçalho, mas é posicionado após o código das dpendências. Assim, se necessário,
      o desenvolvedor pode introduzir código antes ou depois das inclusões de cabeçalhos.
    %TODO explicar melhor de maneira que não precise de aspas
    \item \textbf{Metadados}: construído pela especificação usada, esse bloco na verdade é
      ...
      bloco, enquanto que metadados de escopo geram vários: um para o ``início'' do escopo, vários
      para os metadados contidos nele, e outro para o ``final'' do escopo. Esses blocos devem definir
      o código necessário para exportar a declaração representada por cada metadado para a máquina
      virtual.
    \item \textbf{Final}: construído pela especificação usada, esse bloco deve conter
      ...
      (aquelas vista na seção \ref{cao:conceitos:apis}) do módulo gerado.
