...
\label{cap:atividades}
  ...
  %TODO uniformizar espaçamento em volta dos trechos de código conforme
  %     fizemos na parte de conceitos
  ...
  % TODO: Talvez seja legal estruturar as seções abaixo em itens...
  % TODO: Also, tentar deixar as notas de rodapé menos caóticas
  ...
  implementação de sua máquina virtual em \C{}, ou até \CXX{}. Além disso, como \CXX{}
  ...
  Como os projetos do USPGameDev usavam CMake\footnote{\textit{Software} livre para compilação
  multi-plataforma. Disponível em: \url{http://www.cmake.org}
  (último acesso: 22/11/2013)} para simplificar sua compilação em diversas plataformas e
  compiladores, nós também usamos essa ferramenta. Apesar do nosso sistema fazer parte
  da UGDK, o usuário da \textit{engine} podia configurar ela para ser compilada com
  essa parte dela desativada (caso ele não precise integrar com \script{s}, por exemplo).
  Isso era feito através da manipulação de variáveis de configuração no projeto em
  CMake da UGDK.
  
  A equipe do USPGameDev contém membros que aderem a diferentes sistemas operacionais e
  ambientes de desenvolvimento. O mais usual era trabalhar em um Linux (com as distribuições
  Debian\footnote{Disponível em: \url{http://www.debian.org} (último acesso: 22/11/2013)}
  ou Ubuntu\footnote{Disponível em: \url{http://www.ubuntu.com} (último acesso: 22/11/2013)}) -
  nas quais usávamos \lang{g++}\footnote{Compilador para \CXX{} do projeto GNU.
  Disponível em: \url{http://gcc.gnu.org} (último acesso: 28/11/2013)} como compilador - ou
  em um Windows - e adotando o Visual Studio\footnote{Ambiente de desenvolvimento para \CXX{} e algumas
  outras linguagens feito pela Microsoft para Windows, contendo \textit{IDE}, compilador e
  outras ferramentas. Mais informações: \url{http://www.visualstudio.com/}. (último acesso:
  22/11/2013)} 2010 para desenvolver em \CXX{}. Para controle de versão do código fonte compartilhávamos
  o repositório da UGDK, que usava Git\footnote{\textit{Software} Livre para versionamento cooperativo
  de código fonte. Disponível em: \url{http://git-scm.com/} (último acesso em: 22/11/2013)},
  hospedado no GitHub\footnote{Serviço de hospedagem de repositórios Git. Acessível em:
  \url{https://github.com/} (último acesso: 22/11/2013)}.

  A metodologia adotada pelo USPGameDev é uma variação própria de métodos
  ágeis. O trabalho de formatura de Vinicius K. Daros \cite{scrum:00} trata com bastante
  detalhes desse assunto. No que diz respeito a nosso projeto, o importante é
  notar que as metas que seguíamos nas iterações de trabalho estavam mescladas
  com as metas do jogo \textit{Horus Eye} e da \textit{engine} UGDK, de forma
  que a maior parte do que desenvolvíamos era voltado para ser usados nesses
  outros programas.
  ...
  versão mais nova do \lang{g++}, a 4.7. Apesar de existirem versões ainda mais 
  ...
  pelo nosso próprio gerador de \textit{wrappers}, o OPWIG (que explicamos na seção 
  ...
  executar as baterias de testes, avisando com \textit{e-mails} sempre que algo falha.
  ...
  durante o primeiro semestre de trabalho. No segundo, adotamos o uso de
  \textit{milestones} (ou ``marcos'', em português): elaborávamos uma pequena
  aplicação de um usuário fictício do nosso sistema, e desenvolvíamos apenas o
  necessário para que a aplicação dele funcionasse devidamente. Após completar
  uma \textit{milestone}, montávamos uma outra mais complicada e que exigisse
  mais do nosso projeto, e repetíamos o processo. Esse último método revelou
  ser o mais produtivo para nós.
  ...
  gerenciador. Raramente ela será usada diretamente pelo usuário. Alguns métodos importantes de 
  sua interface são:
  ...
  \begin{itemize}
    ...
    \item Carregar módulos, usado pelo gerenciador internamente.
