...
\label{cap:conceitos}
  ...
  \label{cap:conceitos:apis}
    ...
    Assim como em Lua, um \script{} pode ser usado de duas formas pela máquina virtual:
    como uma sequência de operações a serem executadas ou como um módulo, contendo definições
    e operações que podem ser usadas por outros módulos. No entanto, a API \lang{Python} não diferencia
    esses dois usos. Ao importar um \script{} \lang{Python}, ele será executado e quaisquer operações
    descritas nele serão realizadas, assim como quaisquer definições se tornarão acessíveis
    ...
    objeto que contenha ou execute essa função, ou definir um módulo pela API que o faça. De forma análoga 
    a \lang{Lua}, para criar um módulo desses também é necessário inicializá-lo com código \C{},
