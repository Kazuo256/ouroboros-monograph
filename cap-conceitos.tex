...
\label{cap:conceitos}
  ...
  \label{cap:conceitos:apis}
    ...
    Para usar o módulo, caso você esteja incorporando \lang{Python} em sua aplicação
    basta compilar ele junto com seu programa, e chamar a função de inicialização 
    \lang{initteste} após inicializar a máquina virtual \lang{Python}. Caso contrário,
    basta compilar o módulo numa biblioteca dinâmica - mas não vamos entrar em detalhes
    neste caso pois para o escopo deste projeto, somente usamos módulos nativos em 
    programas que estão incorporando as máquinas virtuais.
