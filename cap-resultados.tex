...
da \lang{libouroboros} na forma outras bibliotecas separaads: \lang{libouroboros-lua}
...
%TODO seria legal que esse exemplo mostrasse todos os arquivos necessários para ele
%     funcionar, desde o CMake até os scripts
...
pega o valor de uma variável de dentro dele (linha 15), muda o valor de outra variável (linha 18),
...
Para esse exemplo funcionar, basta ter um \script{} chamado \lang{exemplo}
na pasta \lang{/scripts/}, na raiz da onde o programa será executado. A extensão
do arquivo deverá ser \lang{.lua} ou \lang{.py} para \lang{Lua} e \lang{Python}
respectivamente (exemplo: \lang{./scripts/exemplo.py}). Tal \script{} deverá
ter uma váriavel com um valor numérico de ponto flutuante chamada
\lang{VariavelNumerica}, e uma função chamada \lang{Funcao} que não recebe
nenhum argumento (no exemplo, o valor de resultado da função não importa
pois não é usado).

Você pode ver a robustez da OPA e de sua generalização das linguagens fazendo o seguinte:
escreva o \script{} em uma linguagem, teste, e então troque o \script{} por um na
outra linguagem, mantendo as restrições ditas acima (nome do arquivo, contendo uma
variável, etc), e teste novamente. O programa irá executar do mesmo modo que antes,
mesmo tendo trocado a linguagem do \script{} usado, pois a lógica é a mesma e a OPA
abstrai o uso específico de cada máquina virtual.
...
Onde:
\begin{itemize}
  \item \textbf{opwig-python} é o OPWIG em questão. Você precisa especificar a
        linguagem para a qual você quer exportar (no caso foi \lang{python}, mas
        poderia ser \lang{lua} também)
