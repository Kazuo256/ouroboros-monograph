...
\begin{description}
  ...
  \item[\textit{Milestone} 1:] Já na segunda \textit{milestone}, adicionamos:
    \begin{itemize}
      \item \lang{Namespaces} como sub-módulos do módulo principal.
      \item Funções devolvendo \lang{void} - ou seja, que devolvem nenhum valor.
      \item Exportar um módulo a partir de um ou mais arquivos de cabeçalhos.
    \end{itemize}
    
  \item[\textit{Milestone} 2:] Na terceira \textit{milestone} nós testamos para garantir 
    que os \script{s} pudessem importar os sub-módulos diretamente, além de adicionar o
    seguinte:
    \begin{itemize}
      \item \lang{Namespaces} aninhados (\lang{namespaces} dentro de \lang{namespaces})
        como uma árvore apropriada de sub-módulos.
      \item Váriaveis globais de tipos primitivos, possivelmente constantes (o \lang{const} do \CXX{}).
    \end{itemize}
  ...
  \item[\textit{Milestone} 4:] Esta \textit{milestone} foi a única que saiu do padrão das outras
    \textit{milestone}. Em vez de criar um programa para testar funcionalidade que iríamos adicionar
    no OPWIG, resolvemos arrumar algumas outras partes do sistema:
    \begin{itemize}
      \item Separamos a implementação do OPWIG para cada linguagem de \script{} do próprio OPWIG.
  ...
  \item[\textit{Milestone} 5:] Finalmente na sexta \textit{milestone} resolvemos implementar algo
    importante que estava faltando: a conversão de classes, para possibilitar que atributos, 
    variáveis, paramêtros de funções e seus valores devolvidos pudessem ser tipos complexos
    (exportados pelo usuário) além dos tipos primitivos.
    
\end{description}

\subsection{Rodando a \textit{Milestone}}
Agora vamos explicar melhor como compilar e rodar uma \textit{milestone}. Como cada 
\textit{milestone} contém a anterior, somente iremos exemplificar a última delas.

Para compilar a partir da pasta raiz do repositório das \textit{milestones} basta rodar:
...
\begin{figure}[ht]
      ...
      \begin{subfigure}{.8\textwidth}
        \begin{center}
          \includegraphics[width=.5\textwidth]{ssMilestone.png}
          \vspace{1em}
...
E finalmente, os arquivos de cabeçalho da \textit{milestone}:
